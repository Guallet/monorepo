...
describe('AmountInput text handling', () => {
  ...
  it('pads on blur and trims trailing zeros for editing', () => {
    expect(formatAmount(null, 2, true)).toBe('');
    expect(formatAmount(2, 0, true)).toBe('2');
    expect(formatAmount(2, 2, true)).toBe('2.00');
    expect(formatAmount(2, 2, false)).toBe('2');
    expect(formatAmount(-2.5, 3, true)).toBe('-2.500');
    expect(formatAmount(-2.5, 3, false)).toBe('-2.5');
  });
});
